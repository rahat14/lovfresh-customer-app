...
class SpecialFoodMenuListFragment(i: Int) : Fragment(), ProductListAdapter.Interaction {
    ...
    var type = i
    private  lateinit var  productListCall: Call<ProductListResponse>
    ...
    private fun loadProductList(id: Int, page: Int) {
        ...
        if(type == 1 ){
            productListCall =
                ApiProvider.dataApi.fetchProductList(HelperClass.getSelectedVendorID(), id, 10, page, "SPL" )
        }else {
            productListCall =
                ApiProvider.dataApi.fetchProductList(HelperClass.getSelectedVendorID(), id, 10, page, "SCH" )
        }
